...
(function () {
  ...
  // Wait for header to be loaded before updating navbar
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const users = getUsers();
    const adminExists = users.find((u) => u.email === "admin@ecyclehub.com");
    if (!adminExists) {
      users.push({
        name: "Admin",
        email: "admin@ecyclehub.com",
        password: "admin123",
      });
      saveUsers(users);
    }
    ...
    checkSession();
    waitForHeader(); // Wait for header to load before updating navbar
    restrictActions();
    handleLogin();
    handleSignup();
  });
