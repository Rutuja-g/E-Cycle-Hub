...
*/
  ...
  async function loadProducts() {
    ...
    // try localStorage
    const ls = localStorage.getItem(PRODUCTS_STORAGE_KEY);
    if (ls) {
      try {
  ...
  function renderProducts(list, gridEl, limit = DEFAULT_MAX_FEATURED) {
    ...
    items.forEach((p) => {
      ...
      article.innerHTML = `
        ...
        )}" alt="${Utils.escapeAttr(p.name)}" loading="lazy"/>
        <div class="card-body">
          <div style="min-width:0">
            <h3 class="card-title">${Utils.escapeHtml(p.name)}</h3>
            <p class="price">$${Number(p.price).toLocaleString()}</p>
          </div>
          <div style="display:flex;justify-content:center;gap:0.5rem;">
            <a class="btn ghost" href="product.html?id=${encodeURIComponent(
              p.id
            )}" aria-label="View details for ${Utils.escapeAttr(
        p.name
      )}">View Details</a>
          </div>
        </div>
