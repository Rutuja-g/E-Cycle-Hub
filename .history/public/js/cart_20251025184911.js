...
(function () {
  ...
  function renderCart() {
    ...
    cart.forEach((item) => {
      ...
      const itemTitle = Utils.createElement("h4");
      itemTitle.textContent = item.name || "Unnamed";
      const itemSubtext = Utils.createElement("p");
      itemSubtext.innerHTML = `$${Utils.formatPrice(
        item.price
      )} <span style="color:var(--muted)">× ${item.quantity}</span>`;
      itemMeta.appendChild(itemTitle);
      itemMeta.appendChild(itemSubtext);

      const quantityControls = Utils.createElement("div", "qty-controls");
      const decreaseBtn = Utils.createElement("button");
      decreaseBtn.type = "button";
      decreaseBtn.innerText = "−";
      decreaseBtn.title = "Decrease quantity";
      decreaseBtn.dataset.action = "decrease";
      const quantityDisplay = Utils.createElement("span");
      quantityDisplay.textContent = item.quantity;
      quantityDisplay.style.minWidth = "28px";
      quantityDisplay.style.textAlign = "center";
      const increaseBtn = Utils.createElement("button");
  ...
  function setupEventListeners() {
    ...
    checkoutBtn.addEventListener("click", function () {
      ...
      if (!isLoggedIn) {
        Utils.showToast("Please log in to continue checkout.", "info");
    ...
    clearBtn.addEventListener("click", function () {
      ...
      renderCart();
      Utils.showToast("Cart cleared successfully!", "success");
    });
  }
